...
bucket_to_bucket = GCSToGCSOperator(
    task_id='copy_file',
    source_bucket='airflow_12',
    source_object='dim_customer.csv',
    destination_bucket=BUCKET_NAME,
    destination_object='dim_customer_move.csv',
    dag = dag)

loading_into_stage = BashOperator(
    task_id = "loading_file",
    bash_command = "bq load --source_format=CSV --skip_leading_rows=1 airflow_stage_dev.customer gs://airflow_12/dim_customer.csv",
    dag = dag)
